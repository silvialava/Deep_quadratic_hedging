...
class RecSolver(object):
    ...
    def loss_fn(self, inputs, training):
        ...
        dw, x, rn, lambdas = inputs
        y_terminal = self.model(inputs, training)
        delta = y_terminal - self.bsde.g_tf(self.bsde.total_time, x[:, :, -1], rn[:,:,-1], lambdas[:,:,-1])
    ...
    def relative_loss(self, inputs, training):
        ...
        delta = y_terminal - self.bsde.g_tf(self.bsde.total_time, x[:, :, -1], rn[:,:,-1], lambdas[:,:,-1])
        denominator = tf.math.maximum(tf.math.abs(y_terminal), tf.math.abs( self.bsde.g_tf(self.bsde.total_time, x[:, :, -1], rn[:,:,-1], lambdas[:,:,-1])))
...
class NonsharedModel(tf.keras.Model):
    ...
    def save_model(self, path_dir = 'Model2_MV'):
    ...
    def load_model(self, path_dir = 'Model2_MV'):
    ...
    def call(self, inputs, training):
        ...
        for t in range(0, self.bsde.num_time_interval-1):
            y = y - self.bsde.delta_t * (
                self.bsde.f_tf(time_stamp[t], x[:, :, t], y, z, rn[:,:,t], lambdas[:,:,t])
        ...
        # terminal time
        y = y - self.bsde.delta_t * self.bsde.f_tf(time_stamp[-1], x[:, :, -2], y, z, rn[:,:,-2], lambdas[:,:,-2]) + \
            tf.reduce_sum(z * dw[:, :, -1], 1, keepdims=True)
        return y         

    def predict_step(self, data):
        dw, x, rn, lambdas = data[0]
        time_stamp = np.arange(0, self.eqn_config.num_time_interval) * self.bsde.delta_t
        all_one_vec = tf.ones(shape=tf.stack([tf.shape(dw)[0], 1]), dtype=self.net_config.dtype)
        y = all_one_vec * self.y_init
        z = tf.matmul(all_one_vec, self.z_init)        
        ...
        return dw, x, rn, lambdas, history     
