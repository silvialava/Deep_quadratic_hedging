...
## Class NonsharedModel to simulate trajectories
    ...
    def f_tf(self, t, x, y, z, rn, lambdas):
        ...
        diagV =  tf.map_fn(tf.linalg.diag, tf.math.sqrt(tf.math.abs(x[:, self.dim:])))
        ...
        return tf.expand_dims((- tf.linalg.diag_part(tf.tensordot(theta, tf.transpose(z[:, :self.dim]), axes = 1)) + tf.linalg.diag_part(tf.tensordot(lambdas[:, self.dim:], tf.transpose(z[:, self.dim:]), axes = 1)) / tf.squeeze(rn)), axis = 1)
    ...
    def g_tf(self, t, x, rn, lambdas):
        ...
        return np.exp(- self.r * self.total_time) * tf.math.maximum(tf.reduce_sum(x[:, :self.dim], axis = 1,  keepdims=True) -  self.dim * self.strike, 0) 
