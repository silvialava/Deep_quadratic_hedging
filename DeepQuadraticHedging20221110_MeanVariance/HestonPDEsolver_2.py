...
def MCS_scheme(m, m1, m2, N, T, U_0, delta_t, eta, rho, sigma, r_d, r_f, kappa_vec, theta_vec, b, b_0, b_1, b_2, Vec_s, Vec_v, Delta_s, Delta_v):
    start = datetime.now()
    
    A_0, A_1 = make_fixed_matrices(N, T, m1, m2, m, rho, sigma, r_d, r_f, Vec_s, Vec_v, Delta_s, Delta_v)
    ...
    for n in range(0, N):
        ...
        kappa_n = kappa_vec[n]
        theta_n = theta_vec[n]
        A_2 = make_A2(N, T, m1, m2, m, rho, sigma, r_d, r_f, kappa_n, theta_n, Vec_s, Vec_v, Delta_s, Delta_v)
        A = A_0 + A_1 + A_2
        lhs_1 = csc_matrix(I - eta * delta_t * A_1)
        inv_lhs_1 = inv(lhs_1)
...
def make_A2(N, T, m1, m2, m, rho, sigma, r_d, r_f, kappa_n, theta_n, Vec_s, Vec_v, Delta_s, Delta_v):
    ...
    for j in range(m2 - 1):
        for i in range(m1 + 1):
            temp_n = kappa_n * (theta_n - Vec_v[j])
            temp2 = 0.5 * sigma ** 2 * Vec_v[j]
            if Vec_v[j] > 1.:
                for k in l_9a:
                    A_2[i + (j + 1) * (m1 + 1), i + (m1 + 1) * (j + 1 + k)] += temp_n * alpha_v(j, k, Delta_v)
                for k in l_10:
                    A_2[i + (j + 1) * (m1 + 1), i + (m1 + 1) * (j + 1 + k)] += temp2 * delta_v(j - 1, k, Delta_v)
            if j == 0:
                for k in l_9c:
                    A_2[i, i + (m1 + 1) * k] += temp_n * gamma_v(j, k, Delta_v)
            else:
                for k in l_10:
                    A_2[i + j * (m1 + 1), i + (m1 + 1) * (j + k)] += (temp_n * beta_v(j - 1, k, Delta_v) + temp2 * delta_v(j - 1, k, Delta_v))
...
def make_fixed_matrices(N, T, m1, m2, m, rho, sigma, r_d, r_f, Vec_s, Vec_v, Delta_s, Delta_v):
