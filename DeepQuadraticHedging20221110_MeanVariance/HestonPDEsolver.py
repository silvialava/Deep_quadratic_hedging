...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 31 15:53:00 2022
...
This script takes the code from

https://github.com/redbzi/NM-Heston

for solving the Heston PDE. 

It has been modified so to solve the PDE arising in the mean-variance
hedging, strating from the modification of the Heston model as in

[Černý, A., & Kallsen, J. (2008)]

In particular, some of the coefficients of the PDE are time-dependent.
...
def F(n, omega, An, b, r_f, delta_t):
    return np.matmul(An, omega) + b * exp(r_f * delta_t * n)

def F_0(n, omega, A_0, b_0, r_f, delta_t):
    return np.matmul(A_0, omega) + b_0 * exp(r_f * delta_t * n)


def F_1(n, omega, A_1, b_1, r_f, delta_t):
    return np.matmul(A_1, omega) + b_1 * exp(r_f * delta_t * n)


def F_2(n, omega, A_2n, b_2, r_f, delta_t):
    return np.matmul(A_2n, omega) + b_2 * exp(r_f * delta_t * n)
...
def MCS_scheme(m, N, U_0, delta_t, eta, A, A_0, A_1, A_2, b, b_0, b_1, b_2, r_f):
    ...
    for n in range(0, N):
        lhs_1 = csc_matrix(I - eta * delta_t * A_1)
        inv_lhs_1 = inv(lhs_1)
        lhs_2 = csc_matrix(I - eta * delta_t * A_2[n, :, :])
        inv_lhs_2 = inv(lhs_2)
        Y_0 = U[:, n] + delta_t * F(n - 1, U[:, n], A[n, :, :], b, r_f, delta_t)
        rhs_1 = Y_0 + eta * delta_t * (b_1 * exp(r_f * delta_t * n) - F_1(n - 1, U[:, n], A_1, b_1, r_f, delta_t))  #we update b_1
        Y_1 = inv_lhs_1 * rhs_1
        rhs_2 = Y_1 + eta * delta_t * (b_2 * exp(r_f * delta_t * n) - F_2(n - 1, U[:, n], A_2[n, :, :], b_2, r_f, delta_t))  #we update b_2
        Y_2 = inv_lhs_2 * rhs_2
        Y_0_hat = Y_0 + eta * delta_t * (F_0(n, Y_2, A_0, b_0, r_f, delta_t) - F_0(n - 1, U[:, n], A_0, b_0, r_f, delta_t))
        Y_0_tilde = Y_0_hat + (0.5 - eta) * delta_t * (F(n, Y_2, A[n, :, :], b, r_f, delta_t) - F(n - 1, U[:, n], A[n, :, :], b, r_f, delta_t))

        rhs_1 = Y_0_tilde + eta * delta_t * (b_1 * exp(r_f * delta_t * n) - F_1(n - 1, U[:, n], A_1, b_1, r_f, delta_t))  #we update b_1
        Y_1_tilde = inv_lhs_1 * rhs_1
        
        rhs_2 = Y_1_tilde + eta * delta_t * (b_2 * exp(r_f * delta_t * n) - F_2(n - 1, U[:, n], A_2[n, :, :], b_2, r_f, delta_t))  #we update b_2
...
def make_matrices(N, T, m1, m2, m, rho, sigma, r_d, r_f, kappa, theta, Vec_s, Vec_v, Delta_s, Delta_v):
    
    # NOTE: kappa and theta are deterministic functions of time
    # Then A2 and A are functions of time 

    A_0 = np.zeros((m, m)) 
    A_1 = np.zeros((m, m))
    A_2 = np.zeros((N+1, m, m))
    ...
    for n in range(N):
        for j in range(m2 - 1):
            for i in range(m1 + 1):
                temp_n = kappa[n] * (theta[n] - Vec_v[j])
                temp2 = 0.5 * sigma ** 2 * Vec_v[j]
                if Vec_v[j] > 1.:
                    for k in l_9a:
                        A_2[n, i + (j + 1) * (m1 + 1), i + (m1 + 1) * (j + 1 + k)] += temp_n * alpha_v(j, k, Delta_v)
                    for k in l_10:
                        A_2[n, i + (j + 1) * (m1 + 1), i + (m1 + 1) * (j + 1 + k)] += temp2 * delta_v(j - 1, k, Delta_v)
                if j == 0:
                    for k in l_9c:
                        A_2[n, i, i + (m1 + 1) * k] += temp_n * gamma_v(j, k, Delta_v)
                else:
                    for k in l_10:
                        A_2[n, i + j * (m1 + 1), i + (m1 + 1) * (j + k)] += (temp_n * beta_v(j - 1, k, Delta_v) + temp2 * delta_v(j - 1, k, Delta_v))
                A_2[n, i + j * (m1 + 1), i + j * (m1 + 1)] += - 0.5 * r_d

    #A_2 = csc_matrix(A_2)
    #plt.spy(A_2)
    #plt.show()
    
    A = np.tile(A_0 + A_1, (N+1, 1, 1)) + A_2
    #A = csc_matrix(A)
    #plt.spy(A)
    #plt.show()
    
    # A_0 and A_1 have dim (m, m)
    # A_2 and A have dim (N+1, m, m)
...
def get_price(Vec_s, Vec_v, s, v, price_grid):
    # For each (s, v) I neel to find
    # the interval Vec_s and Vec_v
    # where s and v are located, and then I
    # interpolate the corresponding price
    # found with the PDE solver
